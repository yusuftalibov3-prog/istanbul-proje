...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Buraya kendi Gemini API Key'ini koymalısın
      const genAI = new GoogleGenAI("BURAYA_API_KEYINIZI_YAZIN");
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* Chat Penceresi */}
      {isOpen && (
        <div className="bg-white dark:bg-slate-900 w-80 md:w-96 h-[500px] rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden mb-4 transition-all animate-in slide-in-from-bottom-5">
          ...
          <div ref={scrollRef} className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950">
            ...
            {isLoading && <div className="text-xs text-slate-400 animate-pulse">Yapay zeka düşünüyor...</div>}
          </div>
          ...
          <div className="p-4 bg-white dark:bg-slate-900 border-t dark:border-slate-800 flex gap-2">
            ...
            />
            <button onClick={handleSend} className="bg-indigo-600 text-white w-10 h-10 rounded-xl flex items-center justify-center hover:bg-indigo-700 transition-colors">

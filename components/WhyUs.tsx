    ...
    <section className="bg-slate-50 dark:bg-slate-950 py-24 transition-colors">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-4">Neden İstanbul El Ele?</h2>
          <p className="text-slate-600 dark:text-slate-400">Eski sistemin eksiklerini kapatıyor, modern şehircilikte dayanışmayı dijitalleştiriyoruz.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {/* Klasik Sistemler Kartı */}
          <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800">
            ...
            <ul className="space-y-6">
              {comparison.map((item, idx) => (
                <li key={idx} className="flex items-start text-slate-400 dark:text-slate-500 line-through">
          ...
          {/* İstanbul El Ele Kartı */}
          <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl shadow-indigo-100 dark:shadow-none border border-indigo-100 dark:border-indigo-900/50 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-5">
              <i className="fas fa-check-circle text-8xl text-indigo-600 dark:text-indigo-400"></i>
            </div>
            <h3 className="text-xl font-bold text-indigo-600 dark:text-indigo-400 mb-8 flex items-center">
              <i className="fas fa-check-circle mr-3"></i> İstanbul El Ele
            </h3>
            <ul className="space-y-6">
              {comparison.map((item, idx) => (
                <li key={idx} className="flex items-start text-slate-800 dark:text-slate-200 font-medium">
